...
def salario(valor_hora, horas_mensais):
    ...
    taxa = {"inss": 0.08, "IR": 0.11, "sindicato": 0.05}
    salario_liq = round(salario_bruto -
                        (salario_bruto * taxa["inss"] + salario_bruto *
                        taxa["IR"] + salario_bruto * taxa["sindicato"]), 2)
...
def tinta(metros_pintar):
    ...
    """
    capacidade = 3 * 18
    return int(1 + metros_pintar // (capacidade) 
               if metros_pintar  % (capacidade) 
               > 0 else metros_pintar //  (capacidade)) 
