...
def gago(texto):
    ...
    for index,palavra in enumerate(texto_dividido):
        texto_dividido[index] = f"{palavra[0]}-{palavra}"
    return " ".join(texto_dividido)
...
def rosquinhas(n):
    ...
    """
    mensa = '%d rosquinhas' % n
    if n >= 10:
        mensa = 'muitas rosquinhas'
    return mensa
...
def fixa_primeiro(s):
    ...
    return stringA + stringB.replace(stringA, "*")
