...
def ordenamento_contrario(lista):
    ...
    for index, item in enumerate(lista):
        lista_inv.append(lista[(index + 1) * -1])
    return lista_inv
...
def intercalamento_listas(lista1, lista2):
    ...
    for index, item in enumerate(lista1):
        lista_inter.append(lista1[index])
        lista_inter.append(lista2[index])
...
def media_anual(temperaturas):
    ...
    """
    m_anual = round(sum(temperaturas) / len(temperaturas), 2)
    maior_m = []
    
    for index, item in enumerate(temperaturas):
        if item > m_anual:
            maior_m.append(index)
